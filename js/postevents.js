...
function ambosFiltros(checkeados, wordIn, postEvents) {
  for (let elemento of checkeados) {
    data.events.filter(evento => (evento.category == elemento) && ((evento.name.toLowerCase().includes(wordIn)) ||  (evento.description.toLowerCase().includes(wordIn))))
              .forEach(evento => {
                postEvents += createCard(evento);
              });
  }
  postEvents.length == 0 ? nothingFound(wordIn) : cardContainer.innerHTML = postEvents;
};
// filtro checks
...
check.addEventListener("change",() => {
  ...
  let wordIn = inputBusqueda.value.toLowerCase().trim(); //verificar esto
      ...
      if ( (checkeado.length > 0) && (wordIn == "") ) {
         ...
         ambosFiltros(checkeado, wordIn, postEvents);            
      } else {
          crearTarjetas();
      };
  });
    ...
    data.events.forEach(event => {
      ...
      if(resultados) {
        ...
        nothingFound(wordIn);
            ...
            ambosFiltros(catSelect, wordIn, postEvents);
  } else {
    crearTarjetas();
  };
